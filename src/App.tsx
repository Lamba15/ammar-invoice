...
import { calculateTotals, formatNumber } from './types/invoice';
...
function App() {
  ...
  useEffect(() => {
    if (invoices.length === 0) {
      ...
      setCurrentInvoice(invoices[0].id);
    }
  }, [invoices.length, currentInvoiceId, createInvoice, setCurrentInvoice, invoices]);

  const currentInvoice = getCurrentInvoice();
  const totals = currentInvoice ? calculateTotals(currentInvoice.items) : { totalBeforeDiscount: 0, totalAfterDiscount: 0, discountAmount: 0 };
  ...
  const handleImageUpload = (itemId: string, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        updateItem(currentInvoice.id, itemId, { image: reader.result as string });
      };
  ...
  const handlePrint = async () => {
    if (isGeneratingPdf || !currentInvoice) return;
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-dark)] flex flex-col print:bg-white print:min-h-0">
      {/* Header */}
      ...
      <div className="flex flex-1 print:block">
        ...
        <div className="flex-1 overflow-auto p-8 print:p-0 print:overflow-visible">
          <div className="max-w-[210mm] mx-auto bg-white min-h-[297mm] shadow-2xl p-12 print:shadow-none print:m-0 print:p-0 print:w-full print:max-w-none">
            {/* Document Header */}
              ...
              <div className="text-right flex flex-col items-end space-y-4">
                <div className="flex flex-col items-end">
                  ...
                  <input
                    ...
                    onChange={(e) => updateInvoice(currentInvoice.id, { clientName: e.target.value })}
                ...
                <div className="flex flex-col items-end">
                  ...
                  <input
                    ...
                    onChange={(e) => updateInvoice(currentInvoice.id, { projectName: e.target.value })}
                ...
                <p className="text-xs text-gray-500 mt-2">INSTAGRAM/{currentInvoice.instagramHandle}</p>
              </div>
            </header>
            ...
            <div className="mb-12">
                    ...
                    <tr key={item.id} className="border-b border-gray-200 group hover:bg-gray-50 transition-colors print:hover:bg-transparent">
                      <td className="py-4 align-top">
                        <input
                          ...
                          onChange={(e) => updateItem(currentInvoice.id, item.id, { serialNumber: e.target.value })}
                        />
                      </td>
                      <td className="py-4 align-top">
                        <div
                      ...
                      <td className="py-4 align-top px-4">
                        <input
                          ...
                          value={item.itemName}
                          onChange={(e) => updateItem(currentInvoice.id, item.id, { itemName: e.target.value })}
                        />
                        <textarea
                          ...
                          onChange={(e) => updateItem(currentInvoice.id, item.id, { description: e.target.value })}
                          onInput={(e) => {
                      ...
                      <td className="py-4 align-top text-center">
                        <input
                          ...
                          value={item.quantity}
                          onChange={(e) => updateItem(currentInvoice.id, item.id, { quantity: parseInt(e.target.value) || 0 })}
                        />
                      </td>
                      <td className="py-4 align-top text-right font-mono text-sm">
                        <input
                          ...
                          onChange={(e) => updateItem(currentInvoice.id, item.id, { priceBeforeDiscount: parseFloat(e.target.value) || 0 })}
                        />
                      </td>
                      <td className="py-4 align-top text-right font-mono text-sm font-bold">
                        <input
                          ...
                          onChange={(e) => updateItem(currentInvoice.id, item.id, { priceAfterDiscount: parseFloat(e.target.value) || 0 })}
                        />
                      </td>
                      <td className="py-4 align-top text-right font-mono text-sm text-gray-400 bg-gray-50 print:bg-transparent pr-2">
                        {formatNumber(item.quantity * item.priceBeforeDiscount)}
                      </td>
                      ...
                      <td className="py-4 align-top text-right print:hidden">
                        <button
                          onClick={() => deleteItem(currentInvoice.id, item.id)}
              ...
              <div className="mt-4 print:hidden">
                <button
                  onClick={() => addItem(currentInvoice.id)}
            ...
            <div className="flex justify-end mb-16 break-inside-avoid">
              <div className="w-64 border-t-2 border-black pt-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-mono text-sm">{formatNumber(totals.totalBeforeDiscount)} EGP</span>
                </div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs uppercase font-bold text-gray-500">Discount Amount</span>
                  <span className="font-mono text-sm text-green-600">- {formatNumber(totals.discountAmount)} EGP</span>
                </div>
                <div className="flex justify-between items-center pt-4 border-t border-gray-200 mt-2">
                  <span className="text-sm uppercase font-bold">Total After Discount</span>
                  <span className="font-mono text-xl font-bold">{formatNumber(totals.totalAfterDiscount)} EGP</span>
                </div>
              </div>
            </div>
